...
// ----------------------------------------------------
  ...
  return (
    <>
    <div className='w-[400px] m-auto pt-6'>
        <div className='text-xl font-Popins font-medium pl-7'> Reset password </div>
<div className="relative" id="input">
  <input onChange={(e)=>setinputData(e.target.value) }
    placeholder="Search..." className="block w-[300px] text-sm h-[50px] px-4 text-slate-900 bg-white 
    rounded-[8px] border border-slate-200 appearance-none focus:border-transparent focus:outline focus:outline-2 
    focus:outline-primary focus:ring-0 hover:border-brand-500-secondary- peer invalid:border-error-500 invalid:focus:border-error-500
    overflow-ellipsis overflow-hidden text-nowrap pr-[48px]"
    id="floating_outlined" type="text"
  />
  <label
    className="peer-placeholder-shown:-z-10 peer-focus:z-10 absolute text-[14px] leading-[150%] text-primary 
    peer-focus:text-primary peer-invalid:text-error-500 focus:invalid:text-error-500 duration-300 transform -translate-y-[1.2rem]
     scale-75 top-2 z-10 origin-[0] bg-white data-[disabled]:bg-gray-50-background- px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 
     peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-[1.2rem] 
     rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1"
    htmlFor="floating_outlined"
  >
    Search...
  </label>
  <div onClick={handelReset} className="absolute top-3 left-[60%]">
    <svg
...
<div> <Link to={'/login'}> Remember Password ? Login ! </Link> </div>
